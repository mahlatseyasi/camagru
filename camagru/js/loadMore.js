...
function loadMore(lastCamagruId, imagePerPages) {
  if (last != null) {
    lastCamagruId = last;
  }
  ...
  xhr.onreadystatechange = function() {
    if (xhr.readyState == 4 && (xhr.status == 200 || xhr.status == 0) && xhr.responseText != null && xhr.responseText != "") {
      ...
      for (var i = 0; responseJSON[i]; i++) {
        ...
        div.innerHTML =
        "<img onclick=\"showModal2(\'" + responseJSON[i]['img'] + "\');\" class=\"icon removable\" src=\"Cama-gru/" + responseJSON[i]['img'] + "\"></img>" +
        "<div id=\"buttons-like\">" +
  ...
  xhr.open("POST", "./forms/getcamagru.php", true);
  xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
  xhr.send("id=" + lastCamagruId + "&nb=" + imagePerPages);
}
...
function escapeHtml(unsafe) {
 ...
 function showModal2(src) {
   ...
   imgModal.src = 'Cama-gru/' + src;
   imageSelected = 'Cama-gru/' + src;
 }
